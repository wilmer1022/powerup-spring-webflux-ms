...
class UserUseCaseTest {
        ...
        void createUser_ValidUser_ReturnsCreatedUser() {
                ...
                when(userRoleUseCase.getUserRoleByRole("CLIENTE"))
                ...
                StepVerifier.create(userUseCase.createUser(validUser))
                                .expectNextMatches(user -> user.documentNumber().equals(validUser.documentNumber()) &&
        ...
        void createUser_EmailAlreadyExists_ThrowsIllegalArgumentException() {
                ...
                StepVerifier.create(userUseCase.createUser(validUser))
                                .expectErrorMessage(
                                                "El correo electrónico " + validUser.email() + " ya está registrado")
                                .verify();
        }
        ...
        void createUser_DefaultRoleNotFound_ThrowsIllegalStateException() {
                ...
                StepVerifier.create(userUseCase.createUser(validUser))
                                .expectErrorMessage("El rol por defecto 'CLIENTE' no existe")
                                .verify();
        }
        ...
        @Test
        void whenValidCredentials_thenSignInSuccessfully() {
                when(userRepository.getUserByEmail(validUser.email())).thenReturn(Mono.just(validUser));
                when(userRepository.validateUserPassword(anyString(), eq(validUser.password())))
                                .thenReturn(Mono.just(true));

                StepVerifier.create(userUseCase.signInUser(validUser.email(), validUser.password()))
        ...
        @Test
        void whenInvalidCredentials_thenSignInFails() {
                when(userRepository.getUserByEmail(validUser.email())).thenReturn(Mono.just(validUser));
                when(userRepository.validateUserPassword(anyString(), eq(validUser.password())))
                                .thenReturn(Mono.just(false));
                
                StepVerifier.create(userUseCase.signInUser(validUser.email(), validUser.password()))
                                .expectErrorMessage("El correo electrónico y/o contraseña son incorrectos")
                                .verify();
        }
