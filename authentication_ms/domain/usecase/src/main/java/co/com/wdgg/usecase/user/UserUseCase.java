    ...
    return Mono.just(user)
            .flatMap(u -> {
                ...
                return userRepository.getUserByEmail(u.email())
                        .flatMap(userRetrieved -> Mono.error(new IllegalArgumentException("El correo electrónico " + u.email() + " ya está registrado")))
                        .switchIfEmpty(
                            Mono.defer(() ->
                                userRoleUseCase.getUserRoleByRole("USER")
                                    .switchIfEmpty(Mono.error(new IllegalStateException("El rol por defecto 'USER' no existe")))
                                    .flatMap(userRole -> {
